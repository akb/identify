...
var files = []file{
	...
	{
		url:    "https://saucelabs.com/downloads/sc-4.5.4-osx.zip",
		name:   "sauce-connect.zip",
		rename: []string{"sc-4.5.4-osx", "sauce-connect"},
	},
}
...
func addChrome(ctx context.Context, latestChromeBuild string) error {
	...
	latestChromePackage := path.Join(prefixMac, latestChromeBuild, chromeFilename)
	...
	latestChromeDriverPackage := path.Join(prefixMac, latestChromeBuild, chromeDriverFilename)
	...
	files = append(files, file{
		...
		url:    cpAttrs.MediaLink,
		rename: []string{"chromedriver_mac64/chromedriver", "chromedriver"},
	})
	return nil
}
...
func addFirefox(desiredVersion string) {
	if desiredVersion == "" {
		files = append(files, file{
			...
			name:    "firefox-nightly.dmg",
			browser: true,
		})
	} else {
		files = append(files, file{
			// This is a recent nightly. Update this path periodically.
			url:     "https://download-installer.cdn.mozilla.net/pub/firefox/releases/" + url.PathEscape(desiredVersion) + "/mac/en-US/firefox-" + url.PathEscape(desiredVersion) + ".zip",
			name:    "firefox.zip",
			browser: true,
		})
	}
...
func main() {
	...
	if err := addLatestGithubRelease(ctx, "mozilla", "geckodriver", "geckodriver-.*macos.tar.gz", "geckodriver.tar.gz"); err != nil {
